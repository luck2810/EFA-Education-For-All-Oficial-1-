...
function popularMaterias(select, deficienciaId, textoPadrao, lista, valorAtual) {
  ...
  lista.forEach(function (materia) {
    if (deficienciaId && materia.deficienciaId !== deficienciaId) return;
    var opcao = document.createElement("option");
...
function atualizarSelects() {
  ...
  popularDeficiencias(document.getElementById("campo-materia-deficiencia"), "Selecione a deficiência", listaDeficiencias);
...
function excluirDeficiencia(deficiencia) {
  var materiasVinculadas = TODAS_MATERIAS.filter(function (materia) {
    return materia.deficienciaId === deficiencia.id;
  });
  if (materiasVinculadas.length) {
    alert("Esta deficiência possui " + materiasVinculadas.length + " matéria(s) vinculada(s). Exclua ou mova as matérias antes de excluir a deficiência.");
    return;
  }
  if (!confirm('Excluir a deficiência "' + deficiencia.nome + '"? As aulas já criadas manterão o nome salvo.')) return;
  firebase.firestore().collection("deficiencias").doc(deficiencia.id).delete().then(function () {
    cancelarEdicaoDeficiencia();
  }).catch(function (erro) {
    console.error("Erro ao excluir deficiência:", erro);
    alert("Não foi possível excluir a deficiência. " + (erro && erro.message ? "Detalhe: " + erro.message : "Tente novamente."));
  });
}
  ...
  DEFICIENCIAS_SUGERIDAS.forEach(function (sugerida) {
    ...
    sugerida.materias.forEach(function (nomeMateria, indice) {
      var jaExiste = TODAS_MATERIAS.some(function (materia) {
        return materia.deficienciaId === deficienciaId && normalizar(materia.nome) === normalizar(nomeMateria);
      });
      if (jaExiste) return;
...
function salvarMateria(evento) {
  ...
  var campoDeficiencia = document.getElementById("campo-materia-deficiencia");
  var campoAtiva = document.getElementById("campo-materia-ativa");
  var campoOrdem = document.getElementById("campo-materia-ordem");
  if (!campoNome || !campoDeficiencia || !campoAtiva || !campoOrdem) {
  ...
  if (!deficienciaId) {
    alert("Selecione a deficiência à qual a matéria pertence.");
    return;
  }

  var duplicada = TODAS_MATERIAS.some(function (materia) {
    return materia.id !== materiaEmEdicao && materia.deficienciaId === deficienciaId && normalizar(materia.nome) === normalizar(nome);
  });
  if (duplicada) {
    alert("Já existe uma matéria com esse nome nesta deficiência.");
    return;
  }

  var ordem = parseInt(campoOrdem.value, 10);
  if (isNaN(ordem)) {
    ordem = TODAS_MATERIAS.filter(function (materia) { return materia.deficienciaId === deficienciaId; }).length + 1;
  }
...
function iniciarEdicaoMateria(materia) {
  ...
  var campoDeficiencia = document.getElementById("campo-materia-deficiencia");
  atualizarSelects();
  var temOpcao = Array.prototype.some.call(campoDeficiencia.options, function (op) { return op.value === materia.deficienciaId; });
  if (!temOpcao && materia.deficienciaId) {
    var opcao = document.createElement("option");
    opcao.value = materia.deficienciaId;
    opcao.textContent = nomeDaDeficiencia(materia.deficienciaId) || "(deficiência removida)";
    campoDeficiencia.appendChild(opcao);
  }
  campoDeficiencia.value = materia.deficienciaId || "";
...
function renderizarMaterias() {
  ...
  var visiveis = TODAS_MATERIAS.filter(function (materia) {
    return !deficienciaFiltro || materia.deficienciaId === deficienciaFiltro;
  });
  visiveis.forEach(function (materia) {
    ...
    meta.textContent = "Deficiência: " + (nomeDaDeficiencia(materia.deficienciaId) || "(removida)") + " · Ordem: " + materia.ordem;
